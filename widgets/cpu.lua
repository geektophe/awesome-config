...
function icon()
    if _cpuicon == nil then
        _cpuicon = widget_new({ type = "imagebox" })
        _cpuicon.image = image(beautiful.widget_cpu)
    end
    return _cpuicon
end
...
function loadwidget(wide)
    if _loadwidget == nil then
        _loadwidget = blingbling.classical_graph.new()
        ...
        _loadwidget:set_tiles_color("#00000022")
        _loadwidget:set_show_text(true)
        _loadwidget:set_label("$percent %")
        vicious.register(_loadwidget, vicious.widgets.cpu,'$1',2)
    end
    return _loadwidget.widget
end
...
function core1widget(wide)
    if _core1widget == nil then
        ...
        _core1widget:set_filled_color("#00000033")
        vicious.register(_core1widget, vicious.widgets.cpu, "$2")
    end
    return _core1widget.widget
end
...
function core2widget(wide)
    if _core2widget == nil then
        ...
        _core2widget:set_filled_color("#00000033")
        vicious.register(_core2widget, vicious.widgets.cpu, "$2")
    end
    return _core2widget.widget
end
